...
rows = c.execute("SELECT cart_item_id, session_id, address, source_url, listing_title, image_url, details, llm_structured, added_at FROM cart_items ORDER BY added_at DESC LIMIT 3").fetchall()

for i, row in enumerate(rows):
    print(f"=== RECENT ITEM #{i+1} ===")
